...
public class WindowInstance {
	...
	protected TimerTask timeEvictor;
	protected Timer evictorTimer;
	...
	private void initProcessingTimeTrigger(String ft)
	{
		...
		if (t instanceof TimeTrigger)
		{
			...
			triggerTimer.scheduleAtFixedRate(processingTimeTrigger, 0, tt.getPeriod());
			
		}
	}
	private void initTimeEvictor()
	{
		//FIXME: change the value from string to enum
		Evictor e = window.getEvictor();
		
		if (e == null)
			return;
		if (e instanceof TimeEvictor)
		{
			TimeEvictor te = (TimeEvictor) e;
			timeEvictor = new TimerTask() {
			...
			evictorTimer = new Timer();
			evictorTimer.scheduleAtFixedRate(timeEvictor, 0, te.getPeriod());
			
		}
	}
	private synchronized void handleTimeEvictor()
	{
		//Evictors work on active window instances only
		if (opState != OperationalState.active)
			return;
		...
		if (e instanceof TimeEvictor)
		{
			...
			List<StreamElement> toRemove = new ArrayList<>();
			long maxTimestamp = this.getMaxTimestamp();
			for (StreamElement se: earlyContent)
			{
				if (se.getTimestamp()< (maxTimestamp - te.getPeriod()))
				{
					toRemove.add(se);
				}
			}
			earlyContent.removeAll(toRemove);
		}
	}
	...
	{
		// In general, we have to evict before insertion
		handleTupleEvictor(o);
	...
	private void handleTupleEvictor(StreamElement o) {
		//Evictors apply for active window instances only
		if (opState != OperationalState.active)
			return;
		Evictor e = this.window.getEvictor();
		if (e == null)
			return;
		if (! (e instanceof CountEvictor)  && !( e instanceof DeltaEvictor))
			return;
		
		if (e instanceof CountEvictor)
		{
			CountEvictor ce = (CountEvictor)e;
			int numItemsToRemove = (earlyContent.size() - ce.getCount())+1;
			for (int i = 1; i <= numItemsToRemove; i++)
			{
				earlyContent.remove(0);
			}
		}
		else // This is a delta evictor
		{
			DeltaEvictor de = (DeltaEvictor) e;
			List<StreamElement> toRemove = new ArrayList<>();
			for (StreamElement se: earlyContent)
			{
				if (o.getValue() - se.getValue() >= de.getThreshold())
				{
					toRemove.add(se);
				}
			}
